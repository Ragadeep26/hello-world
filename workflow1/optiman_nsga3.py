# -*- coding: utf-8 -*-
"""
Created on Thu Apr 30 14:08:19 2020
...
class Optiman_NSGAIII():
    def __init__(self, problem, N_OUTER_DIVISIONS, N_INNER_DIVISIONS, N_GENERATION=10, initial_population=None):
        self.problem = problem
        self.n_outer_divisions = N_OUTER_DIVISIONS
        self.n_inner_divisions = N_INNER_DIVISIONS
        self.initial_population = initial_population
        self.algorithm = NSGAIII(self.problem, N_OUTER_DIVISIONS, N_INNER_DIVISIONS, variator=SBX())
    ...
    def check_initial_population_size(self):
        ...
        if self.initial_population is not None:
            n_objectives = len(self.problem.objectives.keys())
            n_ref_points_outer = int(comb(n_objectives + self.n_outer_divisions - 1, self.n_outer_divisions))
            n_ref_points_inner = int(comb(n_objectives + self.n_inner_divisions - 1, self.n_inner_divisions))
            n_population = n_ref_points_outer + n_ref_points_inner
            if n_population != self.initial_population.shape[0]:    # check size of the initial population
                out = False
